			...
			<div class="container">
				<div class="title_box">
					<div class="btn_l">
						...
						<a class="btn2" href="/mobile/calendar/deleteCalendarQuestion.do?schedulerId=${calendarInfo.schedulerId}&date=${fn:escapeXml(calendarInfo.date)}"><span><tctl:msg key="comn.del" bundle="common"/></span></a>
						</c:if>						
					</div>
					<div class="btn_r">
						<c:if test="${calendarInfo.modify}">
						<c:if test="${calendarInfo.repeatFlag eq 'on'}">
							<a class="btn2" href="/mobile/calendar/modifyCalendarQuestion.do?schedulerId=${calendarInfo.schedulerId}&date=${fn:escapeXml(calendarInfo.date)}"><span><tctl:msg key="comn.modfy" bundle="common"/></span></a>
						</c:if>
						<c:if test="${calendarInfo.repeatFlag ne 'on'}">
							<a class="btn2" href="/mobile/calendar/modifyCalendar.do?schedulerId=${calendarInfo.schedulerId}&date=${fn:escapeXml(calendarInfo.date)}"><span><tctl:msg key="comn.modfy" bundle="common"/></span></a>
						</c:if>
						</c:if>
					</div>
				</div>
