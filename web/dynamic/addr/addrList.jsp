...
<table cellspacing="0" cellpadding="0" class="TM_addr_list_header">
	...
	<tr>
		...
		<th scope="col">
			<c:if test="${sortBy eq 'company'}">
				<c:if test="${sortDir eq 'asc'}">
					<a href="javascript:;"  onclick="sortAddress('company','desc')" class="sortSelectItem">
					<img src="/design/common/image/icon/ic_bullet_up.gif">
				</c:if>			
				<c:if test="${sortDir eq 'desc'}">
					<a href="javascript:;" onclick="sortAddress('company','asc')" class="sortSelectItem">
					<img src="/design/common/image/icon/ic_bullet_down.gif">
				</c:if>
			</c:if>
			<c:if test="${sortBy ne 'company'}">
				<a href="javascript:;" onclick="sortAddress('company','desc')">
			</c:if>
			<tctl:msg key="addr.table.header.004" bundle="addr"/></a>
		</th>
		<th scope="col"><tctl:msg key="addr.table.header.005" bundle="addr"/>
		</th>
	</tr>
</table>
</div>
	...
	<table cellspacing="0" cellpadding="0" class="TM_addr_list_body">
		...
		<tr onclick="viewAddress2(${members.addrbookSeq}, ${members.memberSeq});">
			...
			<td>
				...
				<tr>
					<td class='treeNodeWrapperContents'>
					<a onclick="sendSingleMail('${fn:escapeXml(members.memberName)}', '${fn:escapeXml(members.memberEmail)}');" href="#" title="${fn:escapeXml(members.memberEmail)}">${fn:escapeXml(members.memberEmail)}</a>
					</td>
				</tr>
				</tbody>
				</table>
			...
			<td>
				...
				<tr>
					<td class='treeNodeWrapperContents'>
					${fn:escapeXml(members.companyName)}
					</td>
				</tr>
				</tbody>
				</table>
			...
			<td>
				<c:if test="${!empty members.officeTel}">
					${fn:escapeXml(members.officeTel)}
				</c:if>
				<c:if test="${empty members.officeTel}">
					<c:if test="${!empty members.homeTel}">
						${fn:escapeXml(members.homeTel)}
					</c:if>
				</c:if>
...
<div id='pageBottomNavi' class='pageNavi pageNaviBottomNone'></div>
</c:if>
