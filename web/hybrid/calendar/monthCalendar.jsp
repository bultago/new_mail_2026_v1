			...
			<div class="container" id="calendarWrap">
				<div class="title_box">
					<div class="date_num" id="datePart">
						<a class="btn_prev" href="/hybrid/calendar/monthCalendar.do?date=${fn:substring(calendar.preday,0,6)}01"></a>
						<span class="date">${thisYear}.${thisMonth}</span>
						<a class="btn_next" href="/hybrid/calendar/monthCalendar.do?date=${fn:substring(calendar.nextday,0,6)}01"></a>
					</div>
				</div>
				<table class="month">
					...
					<c:forEach var="date" items="${calendar.dateList}" varStatus="loop">
						...
						<tr>
							...
							<td class="<c:if test="${loop.count % 7 == 0}"> none </c:if><c:if test="${date eq calendar.thisday}"> cho </c:if><c:if test="${date eq calendar.today}"> today </c:if><c:if test="${(fn:substring(date,4,6) eq thisMonth)&&(loop.count % 7 == 1)}"> hol </c:if><c:if test="${(fn:substring(date,4,6) ne thisMonth)&&(loop.count % 7 == 1)}"> hol_dim </c:if><c:if test="${(fn:substring(date,4,6) ne thisMonth)&&(loop.count % 7 != 1)}"> dim </c:if>">
								<a href="/hybrid/calendar/monthCalendar.do?date=${date}">
				...
				<c:forEach var="calendarInfo" items="${calendar.calendarInfos}">
					<c:choose>
						<c:when test="${calendar.thisday eq calendarInfo.drowStartDate}">
							<li>
								<a class="last" href="/hybrid/calendar/viewCalendar.do?schedulerId=${calendarInfo.schedulerId}&date=${calendar.thisday}">
					...
					<li class="schedule_add">
						<a href="/hybrid/calendar/writeCalendar.do?date=${calendar.thisday}&listType=month"><tctl:msg key="scheduler.title.mobile.add" bundle="scheduler"/></a>
					</li>
				</ul>
			</div>
