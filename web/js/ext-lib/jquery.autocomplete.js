...
jQuery.Autocompleter = function(input, options) {
	...
	$input.bind((jQuery.browser.opera ? "keypress" : "keydown") + ".autocomplete", function(event) {
		...
		switch(event.keyCode) {
			...
			default:				
				...
				if(!jQuery.browser.msie && !roadInterval)roadInterval = setInterval(onChange, 100);
				break;
		}
	}).focus(function(){
	...
	function selectCurrent() {
		...
		$input.val(v);
		hideResultsNow();
		if(!jQuery.browser.msie && !roadInterval)clearInterval(roadInterval);
	...
	function onChange(crap, skipPrevCheck) {
		...
		select.setImeMode((validateHan(currentValue))?"etc":"en");
		...
		if(currentValue != ""){
			request(currentValue, receiveData, function(){select.hide();});
		}
	};
	...
	function trimWords(value) {
		...
		var wordSplit = value.split(options.multipleSeparator);
		var words = [];
		var tempVal;
		for ( var i = 0; i < wordSplit.length; i++) {
			if(wordSplit[i].indexOf(options.multipleSeparatorEtc) > -1){
				tempVal = wordSplit[i].split(options.multipleSeparatorEtc);
				jQuery.each(tempVal, function(i, value) {
					if (jQuery.trim(value)){
						words[words.length] = value;
					}
				});
			} else {
				words[words.length] = wordSplit[i];
			}
		}		
	...
	function lastWord(value) {
		...
		var rword = (words && words.length > 0)?words[words.length - 1]:"";		
		return rword;
	}
	...
	function request(term, success, failure) {
		...
		// if an AJAX url has been supplied, try loading the data now
			...
			jQuery.post(options.url,{keyword:lastWord(term)},function(jdata,textStatus){				
				if(textStatus == "success")	{					
					if(jdata){
						success(term, jdata.addrList);
					}
				}
			},"json");
...
jQuery.Autocompleter.Select = function (options, input, select, config) {
	...
	function init() {
		...
		.css("z-index", "10001")
		.attr("id",jQuery(input).attr("id")+"_actb")
		...
		list = jQuery("<ul/>").appendTo(element).mouseover( function(event) {
			...
			input.focus();
			jQuery(input).css("ime-mode",(imeMode == 'etc')?"active":"auto");			
			return false;
		}).mousedown(function() {
	...
	function fillList() {
		...
		for (var i=0; i < max; i++) {
			if (!data[i])
				continue;
			var formatted = options.formatItem(data[i], i+1, max, data[i], term);
			if ( formatted === false )
				continue;			
