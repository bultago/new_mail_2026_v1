...
jQuery.extend(Datepick.prototype, {
	...
	_newInst: function(target, inline) {
		...
		return {id: id, input: target, // Associated target
			...
			siblings: jQuery([])}; // Created siblings (trigger/append)
	},
	...
	_connectDatepick: function(target, inst) {
		...
		if (appendText) {
			var append = jQuery('<span class="' + this._appendClass + '">' + appendText + '</span>');
		...
		if (showOn == 'button' || showOn == 'both') { // Pop-up date picker when button clicked
			...
			trigger.click(function() {
				if (jQuery.datepick._datepickerShowing && jQuery.datepick._lastInput == target)
					jQuery.datepick._hideDatepick();
				else
					jQuery.datepick._showDatepick(target);
				return false;
			});
		}
	...
	_inlineDatepick: function(target, inst) {
		...
		jQuery('body').append(inst.dpDiv);
		this._updateDatepick(inst);
		// Fix width for dynamic number of date pickers
		inst.dpDiv.width(this._getNumberOfMonths(inst)[1] *
			jQuery('.' + this._oneMonthClass, inst.dpDiv)[0].offsetWidth);
	...
	_isDisabledDatepick: function(target) {
		return (!target ? false : jQuery.inArray(target, this._disabledInputs) > -1);
	},
	...
	_optionDatepick: function(target, name, value) {
		...
		if (arguments.length == 2 && typeof name == 'string') {
			return (name == 'defaults' ? jQuery.extend({}, jQuery.datepick._defaults) :
	...
	_doKeyDown: function(event) {
		var inst = jQuery.datepick._getInst(event.target);
		inst._keyEvent = true;
		var handled = true;
		var isRTL = jQuery.datepick._get(inst, 'isRTL');
		if (jQuery.datepick._datepickerShowing)
			switch (event.keyCode) {
				case 9:  jQuery.datepick._hideDatepick(null, '');
						break; // Hide on tab out
				case 13: var sel = jQuery('td.' + jQuery.datepick._dayOverClass, inst.dpDiv);
						if (sel.length == 0)
							sel = jQuery('td.' + jQuery.datepick._currentClass + ':first', inst.dpDiv);
						if (sel[0])
							jQuery.datepick._selectDay(event.target, inst.cursorDate.getTime(), sel[0]);
						else
							jQuery.datepick._hideDatepick(null, jQuery.datepick._get(inst, 'duration'));
						break; // Select the value on enter
				case 27: jQuery.datepick._hideDatepick(null, jQuery.datepick._get(inst, 'duration'));
						break; // Hide on escape
				case 33: jQuery.datepick._adjustDate(event.target, (event.ctrlKey ?
							-jQuery.datepick._get(inst, 'stepBigMonths') :
							-jQuery.datepick._get(inst, 'stepMonths')), 'M');
						break; // Previous month/year on page up/+ ctrl
				case 34: jQuery.datepick._adjustDate(event.target, (event.ctrlKey ?
							+jQuery.datepick._get(inst, 'stepBigMonths') :
							+jQuery.datepick._get(inst, 'stepMonths')), 'M');
						break; // Next month/year on page down/+ ctrl
				case 35: if (event.ctrlKey || event.metaKey)
							jQuery.datepick._clearDate(event.target);
						handled = event.ctrlKey || event.metaKey;
						break; // Clear on ctrl or command + end
				case 36: if (event.ctrlKey || event.metaKey)
							jQuery.datepick._gotoToday(event.target);
						handled = event.ctrlKey || event.metaKey;
						break; // Current on ctrl or command + home
				case 37: if (event.ctrlKey || event.metaKey)
							jQuery.datepick._adjustDate(event.target, (isRTL ? +1 : -1), 'D');
						handled = event.ctrlKey || event.metaKey;
						// -1 day on ctrl or command + left
						if (event.originalEvent.altKey)
							jQuery.datepick._adjustDate(event.target,
								(event.ctrlKey ? -jQuery.datepick._get(inst, 'stepBigMonths') :
								-jQuery.datepick._get(inst, 'stepMonths')), 'M');
						// Next month/year on alt + left/+ ctrl
						break;
				case 38: if (event.ctrlKey || event.metaKey)
							jQuery.datepick._adjustDate(event.target, -7, 'D');
						handled = event.ctrlKey || event.metaKey;
			...
			jQuery.datepick._showDatepick(this);
		else
			handled = false;
		if (handled) {
	...
	_doKeyPress: function(event) {
		var inst = jQuery.datepick._getInst(event.target);
		if (jQuery.datepick._get(inst, 'constrainInput')) {
			var chars = jQuery.datepick._possibleChars(inst);
	...
	_doKeyUp: function(event) {
		var inst = jQuery.datepick._getInst(event.target);
		try {
			var separator = (jQuery.datepick._get(inst, 'rangeSelect') ?
				jQuery.datepick._get(inst, 'rangeSeparator') :
				(jQuery.datepick._get(inst, 'multiSelect') ?
				jQuery.datepick._get(inst, 'multiSeparator') : ''));
			var dates = (inst.input ? inst.input.val() : '');
			dates = (separator ? dates.split(separator) : [dates]);
			var ok = true;
			for (var i = 0; i < dates.length; i++) {
				if (!jQuery.datepick.parseDate(jQuery.datepick._get(inst, 'dateFormat'),
						dates[i], jQuery.datepick._getFormatConfig(inst))) {
					ok = false;
					break;
				}
			}
			if (ok) { // Only if valid
				jQuery.datepick._setDateFromField(inst);
				jQuery.datepick._updateAlternate(inst);
	...
	_possibleChars: function (inst) {
		var dateFormat = jQuery.datepick._get(inst, 'dateFormat');
		var chars = (jQuery.datepick._get(inst, 'rangeSelect') ?
			jQuery.datepick._get(inst, 'rangeSeparator') :
			(jQuery.datepick._get(inst, 'multiSelect') ?
			jQuery.datepick._get(inst, 'multiSeparator') : ''));
		var literal = false;
	...
	_showDatepick: function(input) {
		...
		if (jQuery.datepick._isDisabledDatepick(input) || jQuery.datepick._lastInput == input) // Already here
			return;
		var inst = jQuery.datepick._getInst(input);
		var beforeShow = jQuery.datepick._get(inst, 'beforeShow');
		extendRemove(inst.settings, (beforeShow ? beforeShow.apply(input, [input, inst]) : {}));
		jQuery.datepick._hideDatepick(null, '');
		jQuery.datepick._lastInput = input;
		jQuery.datepick._setDateFromField(inst);
		if (jQuery.datepick._inDialog) // Hide cursor
			input.value = '';
		if (!jQuery.datepick._pos) { // Position below input
			jQuery.datepick._pos = jQuery.datepick._findPos(input);
			jQuery.datepick._pos[1] += input.offsetHeight; // Add the height
		}
		var isFixed = false;
		...
		if (isFixed && jQuery.browser.opera) { // Correction for Opera when fixed and scrolled
			jQuery.datepick._pos[0] -= document.documentElement.scrollLeft;
			jQuery.datepick._pos[1] -= document.documentElement.scrollTop;
		}
		var offset = {left: jQuery.datepick._pos[0], top: jQuery.datepick._pos[1]};
		jQuery.datepick._pos = null;
		// Determine sizing offscreen
		inst.dpDiv.css({position: 'absolute', display: 'block', top: '-1000px'});
		jQuery.datepick._updateDatepick(inst);
		// Fix width for dynamic number of date pickers
		inst.dpDiv.width(jQuery.datepick._getNumberOfMonths(inst)[1] *
			jQuery('.' + jQuery.datepick._oneMonthClass, inst.dpDiv).width());
		// And adjust position before showing
		offset = jQuery.datepick._checkOffset(inst, offset, isFixed);
		inst.dpDiv.css({position: (jQuery.datepick._inDialog && jQuery.blockUI ?
			'static' : (isFixed ? 'fixed' : 'absolute')), display: 'none',
			left: offset.left + 'px', top: offset.top + 'px'});
		if (!inst.inline) {
			var showAnim = jQuery.datepick._get(inst, 'showAnim') || 'show';
			var duration = jQuery.datepick._get(inst, 'duration');
			var postProcess = function() {
				jQuery.datepick._datepickerShowing = true;
				var borders = jQuery.datepick._getBorders(inst.dpDiv);
				inst.dpDiv.find('iframe.' + jQuery.datepick._coverClass). // IE6- only
					css({left: -borders[0], top: -borders[1],
						width: inst.dpDiv.outerWidth(), height: inst.dpDiv.outerHeight()});
			};
			if (jQuery.effects && jQuery.effects[showAnim])
				inst.dpDiv.show(showAnim, jQuery.datepick._get(inst, 'showOptions'), duration, postProcess);
			else
	...
	_updateDatepick: function(inst) {
		...
		if (inst.input && inst.input[0].type != 'hidden' && inst == jQuery.datepick._curInst)
	...
	_hideDatepick: function(input, duration) {
		...
		if (this._datepickerShowing) {
			...
			var postProcess = function() {
				jQuery.datepick._tidyDialog(inst);
			};
			if (duration != '' && jQuery.effects && jQuery.effects[showAnim])
				inst.dpDiv.hide(showAnim, jQuery.datepick._get(inst, 'showOptions'),
	...
	_checkExternalClick: function(event) {
		if (!jQuery.datepick._curInst)
			return;
		var $target = jQuery(event.target);
		if (!$target.parents().andSelf().is('#' + jQuery.datepick._mainDivId) &&
				!$target.hasClass(jQuery.datepick.markerClassName) &&
				!$target.parents().andSelf().hasClass(jQuery.datepick._triggerClass) &&
				jQuery.datepick._datepickerShowing && !(jQuery.datepick._inDialog && jQuery.blockUI))
			jQuery.datepick._hideDatepick(null, '');
	},
	...
	_doHover: function(id, year, month, td) {
		...
		var date = (year ?
			this._daylightSavingAdjust(new Date(year, month, jQuery(td).text())) : null);
	...
	_selectDay: function(id, timestamp, td) {
		...
		if (rangeSelect)
			...
			inst.stayOpen = true;
		if (inst.stayOpen) {
			jQuery('.datepick td', inst.dpDiv).removeClass(this._currentClass);
	...
	dateStatus: function(date, inst) {
		return jQuery.datepick.formatDate(jQuery.datepick._get(inst, 'dateStatus'),
			date, jQuery.datepick._getFormatConfig(inst));
	},
			...
			while (matches) {
				switch (matches[2] || 'd') {
					...
					case 'm':
						...
						day = Math.min(day, jQuery.datepick._getDaysInMonth(year, month));
						break;
					case 'y':
		...
		for (var row = 0; row < numMonths[0]; row++)
			for (var col = 0; col < numMonths[1]; col++) {
				...
				html += '<div class="' + this._oneMonthClass + (col == 0 ? ' datepick-new-row' : '') + '">' +
					...
					'<div style="border-left:#FFFFFF solid 1px;*border-left:none;"><table class="datepick" cellpadding="0" cellspacing="0"><thead>' +
				...
				for (var dRow = 0; dRow < numRows; dRow++) { // Create date picker rows
					...
					for (var dow = 0; dow < 7; dow++) { // Create date picker days
						...
						html += '<td class="datepick-days-cell' +
							...
							' ' + jQuery.datepick._dayOverClass : '') + // Highlight selected day
...
jQuery.fn.datepick = function(options){
	...
	if (typeof options == 'string' && (options == 'isDisabled' ||
		...
		return jQuery.datepick['_' + options + 'Datepick'].
			apply(jQuery.datepick, [this[0]].concat(otherArgs));
	if (options == 'option' && arguments.length == 2 && typeof arguments[1] == 'string')
		return jQuery.datepick['_' + options + 'Datepick'].
			apply(jQuery.datepick, [this[0]].concat(otherArgs));
	return this.each(function() {
		typeof options == 'string' ?
			jQuery.datepick['_' + options + 'Datepick'].
				apply(jQuery.datepick, [this].concat(otherArgs)) :
			jQuery.datepick._attachDatepick(this, options);
	});
};

jQuery.datepick = new Datepick(); // Singleton instance

jQuery(function() {
	jQuery(document).mousedown(jQuery.datepick._checkExternalClick).
		find('body').append(jQuery.datepick.dpDiv);
});
