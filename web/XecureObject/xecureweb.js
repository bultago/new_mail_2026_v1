...
//var xgate_addr	= "210.124.178.206" + "8:8443:8001,8002,8003,8004,8005,8006,8007,8008,8009,8010,8011";
...
function XecureEncParamObject(action,qs){
	var cipher;
	var actionParam = "";
	if( gIsContinue == 0 ) {
		gIsContinue = 1;
		if( IsNetscape60() )		// Netscape 6.0
			cipher = document.XecureWeb.nsIXecurePluginInstance.BlockEnc(xgate_addr, action, XecureEscape(actionParam),"GET");			
		else {
			cipher = document.XecureWeb.BlockEnc(xgate_addr, action, XecureEscape(actionParam),"GET");
		}
		gIsContinue = 0;
	}
	else {
	...
	action = action + "?q=" + escape_url(cipher);
	// encrypt QueryString
	if( gIsContinue == 0 ) {
		gIsContinue = 1;
		if( IsNetscape60() )		// Netscape 6.0
			cipher = document.XecureWeb.nsIXecurePluginInstance.BlockEnc(xgate_addr, action, XecureEscape(qs),"POST");			
		else{
			cipher = document.XecureWeb.BlockEnc(xgate_addr, action, XecureEscape(qs),"POST");
		}
		gIsContinue = 0;
	}
	else {
	...
	var resultObj = {};
	var encObject = {"p":cipher};
	if(usePageCharset)
		encObject.charset =  document.charset;
	resultObj.param = encObject;
	resultObj.action = action;
	return resultObj;
...
{	
	...
	{		
		...
		else
		{
			...
			document.write('<OBJECT ID="XecureWeb" CLASSID="CLSID:7E9FDB80-5316-11D4-B02C-00C04F0CD404" CODEBASE="http://'+window.location.host+'/XecureObject/xw_install.cab#Version=7,2,2,8" width=0 height=0><param name="lang" value="korean">No XecureWeb PlugIn</OBJECT>');
		}
	}
}
