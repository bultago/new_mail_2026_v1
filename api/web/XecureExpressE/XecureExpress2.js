document.writeln('<object id="MAIL" ');
document.writeln('classid="CLSID:F564BD75-62F5-4400-BBC7-063794EF126C" ');
document.writeln('codebase="http://127.0.0.1:8081/XecureExpressE/v2607/XecureOutMail_Install.cab#Version=2,6,0,7">');
document.writeln('</object>');
    
//alert("document.MAIL"+ MAIL);

//http://127.0.0.1:8081/XecureExpressE/writemail1.html
...
function createMail ( form,indn )
{
//테스트용
//form.From.value = "jepark@softforum.com";
//form.To.value = "jwshim@softforum.com";
//form.Cc.value = "jepark@softforum.com";
//form.From.value = "";
//form.To.value = "";
//form.Subject.value = "";
//form.Content.value = "";
//form.From.value = "dyoon12@moge.go.kr";
//form.To.value = "luxmea86@moge.go.kr";

//테스트용

    if ( !form.chkSign.checked && !form.chkEnc.checked) {
		alert("전자서명과 암호화중 하나 이상 선택하셔야 됩니다.");
		return false;
    }
    ...
    // 3. download template
    templateUrl = "http://127.0.0.1:8081/XecureExpressE/Template/Template.html";
    version = "2.0";
	...
	MAIL.From = form.From.value;
	MAIL.To = form.To.value;
	MAIL.Cc = form.Cc.value;
	MAIL.Bcc = form.Bcc.value;
	//MAIL.ReplyTo = form.From.value;
	MAIL.Subject = form.Subject.value;
    MAIL.Content = form.Content.value;
	MAIL.Date = GetDate();
	//MAIL.SetSize.Limit = ""

	if ( form.filename1.value != "" ) {
        MAIL.Attach(form.filename1.value);
	}
	if ( form.filename2.value != "" ) {
	    MAIL.Attach(form.filename2.value);
	}
	if ( form.filename3.value != "" ) {
	    MAIL.Attach(form.filename3.value);
	}
	...
	//==NPKI
   	...
   	//MAIL.DS_CN = "cn=이일부(lee il boo)0004043500098723,ou=KMB,ou=personal4IB,o=yessign,c=kr";
	//alert("indn=>"+indn);
   	MAIL.DS_CN = "cn=863신은미001,ou=people,ou=전라남도교육청,o=Government of Korea,c=KR";
...
*/	
    ...
    codebase = "http://127.0.0.1:8081/XecureExpressE/xei_install2.cab";//  "http://download.softforum.co.kr/XecureExpressI/xei_install2.cab"; //"http://192.168.10.52:8080/webmail/control/xei_install.cab";
	...
	MAIL.ReadMailErrorUrl = "http://127.0.0.1:8081/XecureExpressE/template/error.html";
    ...
    if ( bannerOpt == 0 )
    {
        ...
        bannerUrl = "http://127.0.0.1:8081/XecureExpressE/xecure_daegu.bmp";
        bannerversion = "1.9";
    ...
    if(MAIL.FailTo.length > 0)
        alert("인증서를 가져오지 못한 수신자 : \r\n " + MAIL.FailTo );

	// 10. Get Mail Message
   	form.mail_msg.value = MAIL.MailMessage;
...
//	document.sendmail.content_view.value = form.mail_msg.value;
	...
	// 13. sendmail
  	...
  	{
  	    alert("보안메일 생성을 완료하였습니다. \n'확인' 버튼을 클릭하시면 메일발송이 이루어집니다.");
