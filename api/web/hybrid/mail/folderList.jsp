		...
		<div class="wrapper">	
			...
			<div class="container">
				<div class="title_box">
					...
					<div class="btn_r"><a href="/hybrid/mail/mailWrite.do" class="btn2" title="<tctl:msg key="mail.write"/>"><span><tctl:msg key="mail.write"/></span></a></div>
				</div>
				<div id="document_body">
					<ul class="normal_type mail_home">
						...
						<li>
							<a href="/hybrid/mail/mailMdnList.do" class="Mdn" title="<tctl:msg key="mail.receivenoti"/>"><tctl:msg key="mail.receivenoti"/></a>
						</li>
						</c:if>
						</c:if>
						</c:forEach>						
						...
						<c:forEach var="folder" items="${userFolders}" varStatus="loop">
							<li class="depth${folder.depth}">
								<a href="javascript:moveMailList('${folder.fullName}');" class="UserFolder" style="padding: 10px 20px 10px 10px;">
									${folder.name}
									<c:if test="${folder.unseenCnt > 0}">
									<strong class="mail_num">${folder.unseenCnt}</strong>
									</c:if>
