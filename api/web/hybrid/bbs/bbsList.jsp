			...
			<div class="container">
				<ul class="normal_type board_home">
					<c:forEach var="bbs" items="${bbsInfoList}" varStatus="loop">
						<li class="${loop.last ? 'last':''} <c:if test="${bbs.bbsDepth > 0}">reply depth${bbs.bbsDepth}</c:if>"><a href="/hybrid/bbs/bbsContentList.do?bbsId=${bbs.bbsId}"><span class="${(bbs.bbsType eq 'secret') ? 'secret' : ((bbs.bbsType eq 'any') ? 'any' : '')}">${bbs.bbsName}</span><c:if test="${bbs.existNew eq true}">&nbsp;&nbsp;<span class="ic_newContent">new</span></c:if></a></li>
					</c:forEach>
				</ul>
			</div>
