	...
	<head>
		...
		<script type="text/javascript">			
			...
			function moveto_page(page) {
				var url="/hybrid/addr/writePrivateAddrList.do";
				var paramArray = [];
			...
			function changeAddr(mode) {
				...
				var url = "";
				if (mode == 'pri') {
					url="/hybrid/addr/writePrivateAddrList.do";
				} else if (mode == 'pub') {
					url="/hybrid/addr/writePublicAddrList.do";
				}
				executeUrl(url, paramArray);
			}
			...
			<c:forEach var="contactGroup" items="${contactGroupVos}" varStatus="loop">				
				addrGroupList.push({id:"addr_group_item_${contactGroup.groupSeq}",name:"${contactGroup.groupName}",fullname:"${contactGroup.groupName}",depth:0,link:"javascript:changeGroup('${contactGroup.groupSeq}')"}); 
			</c:forEach>
		...
		<div id="addr_wrapper" class="wrapper">
			<div class="container">
				...
				<c:if test="${!empty contactInfoVo.groupInfo}">
					<div style="width:100%">
						<p class="addr_private_title">
							<span class="title_name"><tctl:truncateString size="20">${contactInfoVo.groupInfo.groupName}</tctl:truncateString></span>
						</p>
					</div>
				</c:if>
					<div class="btn_r"> 
				...
				<div class="mail_address">
					<div class="menu">
						<ul>
							<li><a href="javascript:changeAddr('pri')" class="on"><tctl:msg key="addr.tree.tab1.title" bundle="addr"/> (${contactInfoVo.totalCount})</a></li>
							<li><a href="javascript:changeAddr('pub')"><tctl:msg key="addr.tree.tab2.title" bundle="addr"/></a></li>
						</ul>
					</div>
					<div class="list">
						<ul class="addrList">
							...
							<li>
                               <a href="#" class="wname">
	                               <input type="checkbox" name="email" value="${fn:escapeXml(member.memberName)}|${fn:escapeXml(member.memberEmail)}"/>
