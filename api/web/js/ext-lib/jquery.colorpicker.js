...
jQuery.fn.SevenColorPicker = function()
{
    ...
    if(!jQuery.SCP_Selecter || (jQuery("#_seven_color_selecter").length == 0))
    {
        var html = '<div><ul>';
        var result = jQuery('<div id="_seven_color_selecter" />');        
        for(var i = 0 ; i < _SCP_COLORS.length ; i++){        	
            html += '<li><a href="javascript:;" ref="#'+_SCP_COLORS[i]+'" style="background-color:#'+_SCP_COLORS[i]+';"></a></li>';
        }
        ...
        {        	
            if(/^#?([0-9a-f]{3}|[0-9a-f]{6})$/i.test(col))
            {
                ...
                jQuery.SCP_Active.css('background',col);
                jQuery.SCP_Target.val(col);
            }
        }
        result.hide().css({'position':'absolute','font':'10px verdana','margin':0,'padding':0,'z-index':'10010'})
    ...
    return this.each(function()
    {
		...
		if(myPicker.attr('ref') != _SCP_FLAG){
			...
			myPicker.css('background-color',jQuery(this).val());
		}
    });
}