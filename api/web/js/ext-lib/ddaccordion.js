...
var ddaccordion={
	...
	urlparamselect:function(headerclass){
		...
		if (headerclass == 'expandable') {
			result[0] = expandable;
		} 
		else {
			if(subexpandable) {
				if (subexpandable.indexOf('_') != -1)
				result = subexpandable.split('_');
				else
				result[0] = subexpandable;
			}
		}
		/*window.location.search.match(new RegExp(headerclass+"=((\\d+)(,(\\d+))*)", "i")) //check for "?headerclass=2,3,4" in URL
		if (result!=null)
