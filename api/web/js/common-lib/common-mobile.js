...
function getFolderListEl(type){
	...
	for ( var i = startCnt; i < endCnt; i++) {
		folderName = getEmptySpace(folderList[i].depth) + escape_tag(folderList[i].name);
...
function folderAction(folderName) {
	if (menuType == 'folder') {
		this.location = "/mobile/mail/mailList.do?folderName="+encodeURIComponent(encodeURIComponent(folderName));
...
function goWrite(){
	document.location = "/mobile/mail/mailWrite.do";
}
