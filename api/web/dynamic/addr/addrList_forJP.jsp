...
<c:forEach var="charater" items="${leadingCharaters}" varStatus="loop">
	<a href="javascript:srchByLeading('${groupSeq}','${charater}');" class="btn_alphabet <c:if test="${leadingPattern == charater}"> btn_alphabet_on"</c:if>"><span>${charater}</span></a>
</c:forEach>
</div>
...
<table cellspacing="0" cellpadding="0" class="TM_addr_list_header">
	...
	<tr>
		...
		<th scope="col">
			...
			<tctl:msg key="addr.info.label.004" bundle="addr"/></a>
		</th>
		<th scope="col">
			<tctl:msg key="addr.info.label.003" bundle="addr"/></a>
		</th>
		<th scope="col">
			<tctl:msg key="addr.info.label.001" bundle="addr"/></a>
		</th>
		<th scope="col">
	...
	<table cellspacing="0" cellpadding="0" class="TM_addr_list_body">
		...
		<tr onclick="viewAddress2(${members.addrbookSeq}, ${members.memberSeq});">
			...
			<td>
				...
				<tr>
					<td class='treeNodeWrapperContents'>
						${fn:escapeXml(members.firstName)}
					</td>
				</tr>
				</tbody>
				</table>
