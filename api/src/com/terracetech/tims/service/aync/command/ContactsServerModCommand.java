...
public class ContactsServerModCommand implements ICommand{
	...
	public ContactsServerModCommand(User user, MobileSyncVO syncVo) {
		this.user = user;
		this.syncVo = syncVo;
	...
	public void process(String deviceId, int windowSize) {
		...
		memberList = manager.readModContactsByDate(userSeq, syncVo.getContactsUpdateSyncTime(), 100);
		updateContactsSyncDate(deviceId);
	}
	
	private void updateContactsSyncDate(String deviceId){
		if(memberList==null || memberList.size()==0){
			syncManager.updateContactsUpdateSyncDate(user, deviceId, DateUtil.getFullDateStr());
		...
		if(StringUtils.isNotEmpty(member.getAddTime())){
			syncManager.updateContactsUpdateSyncDate(user, deviceId, member.getModTime());	
		}else{
			syncManager.updateContactsUpdateSyncDate(user, deviceId, DateUtil.getFullDateStr());
		}
	}
