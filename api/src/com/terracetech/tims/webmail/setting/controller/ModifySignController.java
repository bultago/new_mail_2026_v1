 ...
 * 2. 서명 설정 업데이트
 * 
 * Struts2 Action: ModifySignAction
 * 변환 일시: 2025-10-20
 */
@Controller("modifySignController")
public class ModifySignController {
	...
	public String execute(
		...
		// 서명 수정
		boolean updateResult = updateSign(user, signId, signName, signContent);
		
		if (updateResult) {
			model.addAttribute("result", "success");
			model.addAttribute("message", resource.getMessage("sign.modify.success"));
		} else {
			model.addAttribute("result", "fail");
			model.addAttribute("message", resource.getMessage("sign.modify.failed"));
		}
		
		return "success";
	}

	private boolean updateSign(User user, String signId, String signName, String signContent) throws Exception {
		...
		return settingManager.updateSign(signVo);
	}
