...
<template>
  <div class="flex flex-col h-full bg-white dark:bg-zinc-900" v-if="message">
    ...
    <div class="px-4 py-2 border-b border-legacy-border bg-[#F7F7F7] dark:bg-zinc-900 flex items-center gap-1">
       <Button variant="outline" size="sm" class="h-[28px] bg-white text-[#444] text-[11px] dark:bg-zinc-800 dark:text-gray-200 dark:border-zinc-700" @click="handleBack">
           <ChevronLeft class="h-3.5 w-3.5 mr-1" /> 목록
       </Button>
       
       <div class="h-4 w-[1px] bg-[#CCC] mx-2"></div>

       <Button variant="outline" size="sm" class="h-[28px] bg-white text-[#444] text-[11px] dark:bg-zinc-800 dark:text-gray-200 dark:border-zinc-700">
    ...
    <div class="p-6 flex-1 overflow-auto bg-white dark:bg-zinc-900">
        <div class="border border-legacy-border bg-white dark:bg-zinc-100 shadow-sm max-w-[1000px]">
            ...
            <div class="bg-[#F9F9F9] dark:bg-zinc-800 border-b border-legacy-border p-4">
                <div class="mb-3">
                    <h1 class="text-[18px] font-bold text-[#333] dark:text-white tracking-tight leading-tight">
                        <span class="inline-block px-1.5 py-0.5 rounded bg-[#EEE] dark:bg-zinc-700 text-[#666] dark:text-gray-300 text-[11px] font-normal align-middle mr-2 border border-[#DDD] dark:border-zinc-600">받은메일</span>
                        {{ message.subject }}
                    </h1>
                </div>
                <div class="grid grid-cols-[80px_1fr] gap-y-1 text-[12px]">
                    ...
                    <div class="flex items-center">
                        ...
                        <Button variant="ghost" size="sm" class="h-[20px] px-1 ml-2 text-[10px] text-blue-600 dark:text-blue-400 hover:text-blue-800">+ 주소록추가</Button>
                    </div>
            ...
            <!-- Attachments -->
            <div v-if="message.hasAttachment" class="bg-[#FFFBE6] dark:bg-yellow-900/20 px-4 py-3 border-b border-legacy-border flex items-start gap-4">
                <div class="pt-0.5"><Paperclip class="h-4 w-4 text-[#555] dark:text-gray-400" /></div>
                <div class="flex-1">
                    <div class="text-[12px] font-bold text-[#333] dark:text-gray-200 mb-1">
                        ...
                        <a href="#" class="text-legacy-blue dark:text-blue-400 hover:underline">모두저장</a>
                    </div>
                    <!-- Mock Attachment Item -->
                    <div class="flex items-center gap-2 text-[12px] p-1 hover:bg-[#F0F0F0] dark:hover:bg-zinc-700 rounded cursor-pointer max-w-fit">
