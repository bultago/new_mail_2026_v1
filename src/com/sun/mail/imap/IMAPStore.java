...
public class IMAPStore extends Store 
    ...
    private int port = 143;		// default IMAP port
    ...
    /* added variable by sshyun 2004.03.16 begin */
    private boolean isClose = false;
    /* added variable by sshyun 2010.04.22 end */
	...
	// Get the name of this protocol, as set in registry
	if (url != null)
	...
	if (p.hasCapability("AUTH-LOGIN") ||
	    p.hasCapability("AUTH=LOGIN"))
	    p.authlogin(u, pw);
	else
	    p.login(u, pw);
    }
	...
	IMAPProtocol p = null;
        ...
        synchronized (pool) {
            ...
            if (pool.authenticatedConnections.isEmpty() ||
                ...
                if (debug)
                    System.out.println("DEBUG: no connections in the pool, " +
                                       "creating a new one");
                try {
                ...
                // and remove the store as a response handler.
    ...
    IMAPProtocol getStoreProtocol() throws ProtocolException {
        ...
        synchronized (pool) {
            ...
            if (pool.authenticatedConnections.isEmpty()) {
                if (pool.debug)
                    System.out.println(
			"DEBUG: getStoreProtocol() - no connections " +
            ...
            if (!pool.separateStoreConnection) {
                ...
                if (pool.debug) 
                    System.out.println("DEBUG: getStoreProtocol() -- " +
        ...
        synchronized (pool) {
            if (protocol != null) {
                ...
                if (!isConnectionPoolFull()) {
                    ...
                    if (debug)
                        System.out.println("DEBUG: added an " +
                            "Authenticated connection -- size: " +
                            pool.authenticatedConnections.size());
                } else {
                    if (debug)
                        System.out.println("DEBUG: pool is full, not adding " +
    ...
    void releaseStoreProtocol(IMAPProtocol protocol) {
        ...
        synchronized (pool) {
            ...
            if (!pool.separateStoreConnection) {
                ...
                if (pool.debug)
                    System.out.println("DEBUG: releaseStoreProtocol() -- " +
                        "borrowedStoreConnections: " + 
                        pool.borrowedStoreConnections);
            }
    ...
    private void emptyConnectionPool() {
        ...
        synchronized (pool) {
        	IMAPProtocol tempProtocol = null;
            for (int index = pool.authenticatedConnections.size() - 1;
		    index >= 0; --index) {
                try {                	
                	tempProtocol = ((IMAPProtocol)pool.authenticatedConnections.
                            elementAt(index));
                	tempProtocol.removeResponseHandler(this);
                	if(!isClose)tempProtocol.logout();
                } catch (ProtocolException pex) {};
            }
    ...
    private void timeoutConnections() {
        ...
        synchronized (pool) {
            ...
            if (System.currentTimeMillis() - pool.lastTimePruned > 
                ...
                if (pool.debug) {
                    System.out.println("DEBUG: checking for connections " +
                        "to prune: " +
	...
	try {
        ...
        * added part by sshyun 2010.04.22 begin *
        isClose = true;
        * added part by sshyun 2010.04.22 end *
    ...
    */
    ...
    // 2016-07-27 수정 : diyoo
    // javamail-1.4.7 의 로직으로 변경 
    // 파이널라이저 쓰레드에서 close() 메소드를 호출하는데, getProtocol() 쪽 문제로 예상되어 패치 함.
    public synchronized void close() throws MessagingException{
    	//System.out.println("### "+this.getClass().getName()+" - close() : super.isConnected() : "+super.isConnected());
    	if(!super.isConnected())
    		return;
        ...
        try{
            ...
            //System.out.println("### "+this.getClass().getName()+" - close() : isEmpty : "+isEmpty);
            if(isEmpty)
            {
                cleanup();
                return;
            ...
            protocol = getStoreProtocol();
            //System.out.println("### "+this.getClass().getName()+" - close() : protocol : "+protocol);
            synchronized(pool)
            {
	...
	for (int i = 0; i < fa.length; i++) {
	    ...
	    fa[i] = new IMAPFolder(name, ns[i].delimiter, this);
	}
	return fa;
    }
