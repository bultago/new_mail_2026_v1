package com.terracetech.tims.webmail.note.controller;

import jakarta.servlet.http.HttpServletRequest;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.stereotype.Controller;
import org.springframework.ui.Model;
import org.springframework.web.bind.annotation.RequestParam;
import com.terracetech.tims.webmail.util.SessionUtil;
import com.terracetech.tims.webmail.mailuser.User;
import com.terracetech.tims.webmail.note.manager.NoteManager;
import com.terracetech.tims.webmail.note.vo.NoteVO;

@Controller("noteSendController")
public class NoteSendController {
	@Autowired
	private NoteManager noteManager;

	public String execute(
			@RequestParam(value = "toUserId", required = false) String toUserId,
			@RequestParam(value = "noteContent", required = false) String noteContent,
			HttpServletRequest request,
			Model model) throws Exception {
		User user = SessionUtil.getUser(request);
		NoteVO noteVo = new NoteVO();
		noteVo.setFromUserSeq(Integer.parseInt(user.get(User.MAIL_USER_SEQ)));
		noteVo.setToUserId(toUserId);
		noteVo.setNoteContent(noteContent);
		boolean result = noteManager.sendNote(noteVo);
		model.addAttribute("result", result ? "success" : "fail");
		return "success";
	}
}
