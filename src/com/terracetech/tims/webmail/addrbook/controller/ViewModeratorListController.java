...
/**
 * 주소록 관리자 목록 조회 Controller
 * 
 * 주요 기능:
 * 1. 주소록 관리자(Moderator) 목록 조회
 * 2. 페이징 처리
 * 3. 검색 기능 (이름, 이메일)
 * 4. 이메일 도메인 자동 제거
 * 
 * Struts2 Action: ViewModeratorListAction
 * 변환 일시: 2025-10-20
 */
@Controller("viewModeratorListController")
public class ViewModeratorListController {
	...
	public String executePart(
		...
		// 관리자 목록 조회
		int total = getModeratorTotal(bookSeq, searchType, keyWord);
		List<AddressBookModeratorVO> list = getModeratorList(bookSeq, page, searchType, keyWord);
		AddressBookModeratorVO[] moderatorList = list.toArray(new AddressBookModeratorVO[list.size()]);
	 ...
	 * @throws Exception
	 */
	private int getModeratorTotal(int bookSeq, String searchType, String keyWord) 
			throws Exception {
		return addressBookManager.readAddressBookModeratorListCount(bookSeq, searchType, keyWord);
	}
	 ...
	 * @throws Exception
	 */
	private List<AddressBookModeratorVO> getModeratorList(int bookSeq, int page, 
			String searchType, String keyWord) throws Exception {
		return addressBookManager.readAddressBookModeratorList(bookSeq, page, PAGE_BASE, 
				searchType, keyWord);
	}
