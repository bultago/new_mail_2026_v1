    ...
    public JSONObject copyOrgMember(AddressBookDupMemberVO[] dupMember) {
		...
		int domainSeq = Integer.parseInt(user.get(User.MAIL_DOMAIN_SEQ));
	        ...
	        // orgCode,memberSeq
				...
				String addrAddType = ""; // 중복주소에 대한 선택
				String orgCode = "";     // 해당조직 코드
				int orgUserSeq = 0;      // 조직도에서 선택한 사용자 userSeq
				int groupSeq = 0;        // 추가할 주소록의 groupSeq
				
	            for (int i=0 ; i < dupMember.length ; i++) {
	            	
	            	addrAddType = dupMember[i].getAddrAddType();  //중복주소에 대한 선택(중복추가,덮어쓰기,추가안하기,취소)
	            	orgCode = dupMember[i].getOrgCode();
	            	orgUserSeq = dupMember[i].getUserSeq();
	            	groupSeq = dupMember[i].getGroupSeq();
	            	
	            	if(!("noWrite".equalsIgnoreCase(addrAddType) || "cancel".equalsIgnoreCase(addrAddType))){ // 중복추가,덮어쓰기
		            	...
		            	if("overWrite".equalsIgnoreCase(addrAddType)){  //덮어쓰기시 기존 주소 삭제
		            		addrMember = manager.getExistEmail(userSeq,member.getMemberEmail(),member.getMemberName());
