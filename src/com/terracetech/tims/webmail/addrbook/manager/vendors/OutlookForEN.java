...
public class OutlookForEN implements IEmailVendor {

	private I18nResources resource = null;
	
	public OutlookForEN(I18nResources resource) {
	...
	public StringBuffer getAddrCSVDownload(List<AddressBookMemberVO> list) {
		...
		sb.append(resource.getMessage("addr.outlook.001") + ",");	//이름
		sb.append(resource.getMessage("addr.outlook.002") + ",");	//성
		sb.append(resource.getMessage("addr.outlook.004") + ",");	//전자 메일 주소
		
		sb.append(resource.getMessage("addr.outlook.005") + ",");	//집 번지
		sb.append(resource.getMessage("addr.outlook.006") + ",");	//집 주소 구/군/시
		sb.append(resource.getMessage("addr.outlook.007") + ",");	//집 주소 우편 번호
		sb.append(resource.getMessage("addr.outlook.008") + ",");	//집 주소 시/도
		sb.append(resource.getMessage("addr.outlook.009") + ",");	//집 주소 국가/지역
		sb.append(resource.getMessage("addr.outlook.010") + ",");	//집 전화 번호
		
		sb.append(resource.getMessage("addr.outlook.012") + ",");	//휴대폰
		
		sb.append(resource.getMessage("addr.outlook.013") + ",");	//근무지 주소 번지
		sb.append(resource.getMessage("addr.outlook.014") + ",");	//근무지 구/군/시
		sb.append(resource.getMessage("addr.outlook.015") + ",");	//근무지 우편 번호
		sb.append(resource.getMessage("addr.outlook.016") + ",");	//근무지 시/도
		sb.append(resource.getMessage("addr.outlook.017") + ",");	//근무지 국가/지역
		sb.append(resource.getMessage("addr.outlook.018") + ",");	//근무처 전화
		sb.append(resource.getMessage("addr.outlook.020") + ",");	//회사
		sb.append(resource.getMessage("addr.outlook.021") + ",");	//직함
		sb.append(resource.getMessage("addr.outlook.022"));			//부서
		sb.append("\r\n");
