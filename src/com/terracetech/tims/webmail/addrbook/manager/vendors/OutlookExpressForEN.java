...
public class OutlookExpressForEN implements IEmailVendor {

	private I18nResources resource = null;
	
	public OutlookExpressForEN(I18nResources resource) {
	...
	public StringBuffer getAddrCSVDownload(List<AddressBookMemberVO> list) {
		...
		sb.append(resource.getMessage("addr.outlookExpress.001") + ",");	//ÀüÃ¼ ÀÌ¸§
		sb.append(resource.getMessage("addr.outlookExpress.006") + ",");	//ÀüÀÚ ¸ÞÀÏ ÁÖ¼Ò
		
		sb.append(resource.getMessage("addr.outlookExpress.007") + ",");	//ÁÖ¼Ò(Áý)
		sb.append(resource.getMessage("addr.outlookExpress.008") + ",");	//±¸±º½Ã(Áý)
		sb.append(resource.getMessage("addr.outlookExpress.009") + ",");	//¿ìÆí¹øÈ£(Áý)
		sb.append(resource.getMessage("addr.outlookExpress.010") + ",");	//½Ã/µµ(Áý)
		sb.append(resource.getMessage("addr.outlookExpress.011") + ",");	//±¹°¡(Áý)
		sb.append(resource.getMessage("addr.outlookExpress.012") + ",");	//Áý ÀüÈ­
		
		sb.append(resource.getMessage("addr.outlookExpress.014") + ",");	//ÈÞ´ëÆù
		
		sb.append(resource.getMessage("addr.outlookExpress.016") + ",");	//ÁÖ¼Ò(È¸»ç)
		sb.append(resource.getMessage("addr.outlookExpress.017") + ",");	//±¸/±º/½Ã(È¸»ç)
		sb.append(resource.getMessage("addr.outlookExpress.018") + ",");	//¿ìÆí ¹øÈ£(È¸»ç)
		sb.append(resource.getMessage("addr.outlookExpress.019") + ",");	//½Ã/µµ(È¸»ç)
		sb.append(resource.getMessage("addr.outlookExpress.020") + ",");	//±¹°¡(È¸»ç)
		sb.append(resource.getMessage("addr.outlookExpress.022") + ",");	//È¸»ç ÀüÈ­
		sb.append(resource.getMessage("addr.outlookExpress.024") + ",");	//È¸»ç
		sb.append(resource.getMessage("addr.outlookExpress.025") + ",");	//Á÷ÇÔ
		sb.append(resource.getMessage("addr.outlookExpress.026"));			//ºÎ¼­
		sb.append("\r\n");
