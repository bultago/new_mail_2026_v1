...
public class OutlookExpressForKO implements IEmailVendor {
	...
	public StringBuffer getAddrCSVDownload(List<AddressBookMemberVO> list) {
		...
		sb.append(resource.getMessage("addr.outlookExpress.001") + ",");	//1.전체 이름
		sb.append(resource.getMessage("addr.outlookExpress.006") + ",");	//2.전자 메일 주소
		
		sb.append(resource.getMessage("addr.outlookExpress.007") + ",");	//3.주소(집)
		sb.append(resource.getMessage("addr.outlookExpress.008") + ",");	//4.구군시(집)
		sb.append(resource.getMessage("addr.outlookExpress.009") + ",");	//5.우편번호(집)
		sb.append(resource.getMessage("addr.outlookExpress.010") + ",");	//6.시/도(집)
		sb.append(resource.getMessage("addr.outlookExpress.011") + ",");	//7.국가(집)
		sb.append(resource.getMessage("addr.outlookExpress.012") + ",");	//8.집 전화
		
		sb.append(resource.getMessage("addr.outlookExpress.014") + ",");	//9.휴대폰
		
		sb.append(resource.getMessage("addr.outlookExpress.016") + ",");	//10.주소(회사)
		sb.append(resource.getMessage("addr.outlookExpress.017") + ",");	//11.구/군/시(회사)
		sb.append(resource.getMessage("addr.outlookExpress.018") + ",");	//12.우편 번호(회사)
		sb.append(resource.getMessage("addr.outlookExpress.019") + ",");	//13.시/도(회사)
		sb.append(resource.getMessage("addr.outlookExpress.020") + ",");	//14.국가(회사)
		sb.append(resource.getMessage("addr.outlookExpress.022") + ",");	//15.회사 전화
		sb.append(resource.getMessage("addr.outlookExpress.024") + ",");	//16.회사
		sb.append(resource.getMessage("addr.outlookExpress.025") + ",");	//17.직함
		sb.append(resource.getMessage("addr.outlookExpress.026"));			//18.부서
		sb.append("\r\n");
