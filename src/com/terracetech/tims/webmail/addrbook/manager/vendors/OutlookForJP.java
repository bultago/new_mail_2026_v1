...
public class OutlookForJP implements IEmailVendor {
	...
	public StringBuffer getAddrCSVDownload(List<AddressBookMemberVO> list) {
		...
		sb.append(resource.getMessage("addr.outlook.001") + ",");	//0.이름
		sb.append(resource.getMessage("addr.outlook.002") + ",");	//1.성
		sb.append(resource.getMessage("addr.outlook.004") + ",");	//2.전자 메일 주소
		
		sb.append(resource.getMessage("addr.outlook.005") + ",");	//3.집 번지
		sb.append(resource.getMessage("addr.outlook.006") + ",");	//4.집 주소 구/군/시
		sb.append(resource.getMessage("addr.outlook.007") + ",");	//5.집 주소 우편 번호
		sb.append(resource.getMessage("addr.outlook.008") + ",");	//6.집 주소 시/도
		sb.append(resource.getMessage("addr.outlook.009") + ",");	//7.집 주소 국가/지역
		sb.append(resource.getMessage("addr.outlook.010") + ",");	//8.집 전화 번호
		
		sb.append(resource.getMessage("addr.outlook.012") + ",");	//9.휴대폰
		
		sb.append(resource.getMessage("addr.outlook.013") + ",");	//10.근무지 주소 번지
		sb.append(resource.getMessage("addr.outlook.014") + ",");	//11.근무지 구/군/시
		sb.append(resource.getMessage("addr.outlook.015") + ",");	//12.근무지 우편 번호
		sb.append(resource.getMessage("addr.outlook.016") + ",");	//13.근무지 시/도
		sb.append(resource.getMessage("addr.outlook.017") + ",");	//14.근무지 국가/지역
		sb.append(resource.getMessage("addr.outlook.018") + ",");	//15.근무처 전화
		sb.append(resource.getMessage("addr.outlook.020") + ",");	//16.회사
		sb.append(resource.getMessage("addr.outlook.021") + ",");	//17.직함
		sb.append(resource.getMessage("addr.outlook.022"));			//18.부서
		sb.append("\r\n");
