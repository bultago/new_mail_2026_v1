// $Id: Twofish_Algorithm.java,v 1.1.1.1.4.1 2008-04-02 10:02:59 isle4 Exp $
//
// $Log: not supported by cvs2svn $
// Revision 1.1.1.1  2008/03/27 04:08:07  isle4
//
//
// Revision 1.1.1.1  2008/03/21 08:16:16  isle4
//
//
// Revision 1.1.1.1  2007/12/26 04:50:28  isle4
//
//
// Revision 1.1.1.1  2006/05/13 09:15:24  do
//
//
// Revision 1.3  2004/08/10 07:34:04  dssong
// *** empty log message ***
//
// Revision 1.2  2004/08/10 06:20:57  dssong
// *** empty log message ***
//
...
//...........................................................................
   ...
   private static String toString (byte[] ba, int offset, int length) {
    ...
    private static final byte[] hexStringToByteArray(String data)
    {
        ...
        {
               ch = stringToByte(data.substring(inx, inx+1));
               inx++;
               ch2 = stringToByte(data.substring(inx, inx+1));
               ch = (byte) (ch << 4);
               ch = (byte) (ch & 0xF0);
               ch = (byte) (ch | ch2);
               inx++;
   ...
   static String keyStr = "000102030405060708090A0B0C0D0E0F";
   public static String TwofishEncodeBase64(String plain) {
       byte[] bk = new byte[16];
       bk = hexStringToByteArray(keyStr);

       Object key = null;
       try {
           key = Twofish_Algorithm.makeKey(bk);
       } catch (Exception e) { }

       byte[] pt = null;
       byte[] pt2 = new byte[16];
       try {
           pt = plain.getBytes("UTF-8");
       } catch (Exception e) { }
       for (int i = 0; i < pt.length && i < 16; i++) {
               pt2[i] = pt[i];
       }
       ...
       byte[] cipher = blockEncrypt(pt2, 0, key);
       byte[] b64encoded = BASE64EncoderStream.encode(cipher);

       return new String(b64encoded);
   }

   public static String TwofishDecodeBase64(String cipherBase64) {
       byte[] bk = new byte[16];
       bk = hexStringToByteArray(keyStr);
       ...
       byte[] cipher = BASE64DecoderStream.decode(cipherBase64.getBytes());
       byte[] cpt = Twofish_Algorithm.blockDecrypt(cipher, 0, key);
