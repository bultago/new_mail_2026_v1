...
public class ModifyUserEtcInfoAction extends BaseAction {
...
private static final long serialVersionUID = 20090311L;
	...
	private String userLocale; // NIL or ko or en
	...
	public String execute() throws Exception{		
		...
		if (!"on".equals(searchAllFolder)) {
			searchAllFolder = "off";
		}
		
		if (!"on".equals(hiddenImg)) {
			hiddenImg = "off";
		}
		
		if (!"on".equals(hiddenTag)) {
			hiddenTag = "off";
		}
		
		if (!"on".equals(vcardApply)) {
			vcardApply = "off";
		}
		...
		try {
			...
			settingManager.modifyUserEtcInfo(userSeq, info);
